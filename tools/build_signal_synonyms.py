...
def _build_output_content(canonical_ids: list[str], definitions: dict[str, str], synonyms_map: dict[str, str]) -> str:
    output_lines = [
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build signal synonym mappings.")
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify generated file matches metadata without writing.",
    )
...
def main() -> int:
    ...
    output_content = _build_output_content(canonical_ids, definitions, synonyms_map)

    if args.check:
        if not output_path.exists():
            print("OUT OF DATE: generated file is missing.")
            return 1
        existing = output_path.read_text(encoding="utf-8")
        if existing == output_content:
            print("OK: generated file is up to date.")
            return 0
        print("OUT OF DATE: generated file does not match metadata.")
        return 1
