...
**Conflict rule:** follow desired behavior; log repo deltas with file paths in `packages/specs/docs/program/REPO_COHERENCE_REPORT.md`.
...
- **packages/specs** = canonical specs/contracts/schemas/taxonomy (shared truth)
  - Examples:
    - `packages/specs/docs/specs/phase2-system-overview-v0.1.md`
    - `packages/specs/docs/specs/phase2-offline-judgement-v0.1.md`
...
- **packages/ep** = **EP runtime product** (client-run evaluation pipeline)
  ...
  - Repo reality today: mostly a sandbox target system (TargetLab): `packages/ep/services/targetlab_rag/*`
- **packages/pe** = **PE** (regression/benchmark eval suites that test EP)
  - Must contain: regression suites, benchmarks, CI eval runs, datasets, analysis.
  - Repo reality today: contains substantial EP runtime under `packages/pe/aigov_eval/*` (must migrate).

### Boundary rule (locked)
“Required for a client run?” ⇒ **EP ⇒ `packages/ep`**; otherwise ⇒ **PE ⇒ `packages/pe`**.
...
Repo reality:
- Stage A runner + target adapters + scorers exist today in `packages/pe/aigov_eval/*` (mislocated per boundary).
- Target sandbox exists in `packages/ep/services/targetlab_rag/app.py`.
- Phase2 contracts/specs exist in `packages/specs/docs/specs/phase2-*.md`.

### PE (Product Evaluations)
PE is the harness + datasets that validate EP runtime behavior:
- batch calibration: `packages/pe/aigov_eval/batch_runner.py`
- cases: `packages/pe/cases/calibration/*.json`
- judge tests: `packages/pe/tests/judge/*`
- minimal loop tests: `packages/pe/tests/minimal_loop/*`
...
### Primary goals
1) Enforce repo boundary discipline (EP in `packages/ep`, PE in `packages/pe`, canonical contracts in `packages/specs`).  
...
3) Eval-first development: requirements are driven by the matrix in `packages/specs/docs/program/REQUIREMENTS_EVALS_MATRIX.md`.
...
1) Intake (validated config): `packages/specs/docs/specs/client-intake-variables.md`  
2) Scenario selection + client tailoring → bundle compilation: `packages/specs/docs/specs/phase2-scenario-bundle-v0.1.md`  
3) Stage A execution (no scoring): `packages/specs/docs/specs/phase2-offline-judgement-v0.1.md`  
4) Stage B offline judgement (fail-closed): `packages/specs/docs/specs/phase2-offline-judgement-v0.1.md`  
5) Stage C reporting + exports: `packages/specs/docs/specs/phase2-reporting-exports-v0.1.md`
...
Authoritative list: `packages/specs/docs/program/REQUIREMENTS_EVALS_MATRIX.md`.
...
Canonical in packages/specs:
- `packages/specs/schemas/behaviour_json_v0_phase0.schema.json`
- `packages/specs/schemas/evaluation_criteria/gdpr-evaluation-criteria-v1.0.yaml`
- `packages/specs/scenarios/templates/scenario-template-v1.2.yaml`
- `packages/specs/docs/specs/variable-registry.md`
- `packages/specs/docs/specs/file-registry.md`

Known conflicts to resolve (see coherence report):
- `packages/pe/schemas/behaviour_json_v0_phase0.schema-*.json` duplicates
- `packages/specs/schemas/judge_scenario_instruction_v0.1.json` shape divergence vs phase0 schema

---

## 6) Target repo layout (after refactor)
packages/ep (EP runtime)
...
Specs exist under `packages/specs/projects/akg/*`; runtime integration not present in repos today.
