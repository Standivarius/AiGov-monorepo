...
| AIGov bundle | Deterministic scenario bundle (`manifest.json` v0.1.0 + sha256) per `packages/specs/schemas/deterministic_bundle_manifest_v0_1_0.schema.json` and `packages/specs/docs/contracts/execution/deterministic_bundle_manifest_v0_1_0.md` | Scenario JSON files + manifest | Paths and hashes are verified; fails closed on mismatch or ambiguity | TBD: multi-scenario execution policy | tools/validate_bundle_integrity.py + tools/validate_ep_deterministic_bundle_manifest.py |
...
| LiveRun (M_LiveRun) | seed_instructions_v0_1.md + inspect_task_args_v0_1.md | inspect_logs_v0_1.md + petri_transcripts_v0_1.md | Coordinates Inspect + Petri runs; no additional validation beyond component validators | TBD: runtime orchestration invariants | tools/validate_liverun_inspect_task_args_v0_1.py + tools/validate_liverun_output_artifacts_envelope_v0_1.py (planning-pack fixtures) |
