...
## PR-F: STATUS guard hardening
- STATUS guard now validates STATUS.md edits and blocks conflict markers, stray branch lines, and multiple Next pointers.
- Next: PR-G (Require checks + GitHub Project dashboard + North Star issue).
...
## PR-D: CI proof + logs
- CI now runs the proof command and uploads docs/logs/*.log as an artifact.
- WSL proof log: docs/logs/EVALSET-MIGRATION-SMOKE-v1_20260116_154302.log.
...
- WSL proof: `npm install`; `npx nx run evalsets:migration-smoke` (1 skipped).
  Log: docs/logs/EVALSET-MIGRATION-SMOKE-v1_20260116_004744.log.
- Next (historical): PR-F (Require checks + GitHub Project dashboard + North Star issue).
...
- If pytest deps are missing:
  ...
  (if jsonschema import fails, also install `jsonschema` in the venv).
- WSL proof: `npm install`; `npx nx run evalsets:migration-smoke` (1 skipped).
  Log: docs/logs/EVALSET-MIGRATION-SMOKE-v1_20260115_185556.log.
- Next (historical): PR-B (Codespaces/devcontainer parity).
...
- Next (historical): PR-0/PR-B to land monorepo structure + cross-platform smoke runners.
